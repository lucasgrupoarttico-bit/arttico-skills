...
1. **Meta Ads via MCP oficial**: para Meta Ads, usar PREFERENCIALMENTE as ferramentas `mcp__claude_ai_Meta__ads_*` (MCP oficial integrado ao Claude). Recorrer aos scripts Python da skill `meta-ads-ratos` apenas para operações não suportadas pelo MCP (swap-url-tags, duplicate-campaign, etc.). Para Google Ads e GA4, continuar usando as skills Ratos (google-ads-ratos, ga4-ratos). Nunca usar MCPs de terceiro (fb-ads-mcp-server, adloop, etc.).
