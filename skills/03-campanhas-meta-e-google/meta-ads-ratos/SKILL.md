...
Skill completa para gestao de Meta Ads. Usa o **MCP oficial** (`mcp__claude_ai_Meta__ads_*`) como camada principal e os scripts Python (`facebook-business`) como fallback para operações avançadas não suportadas pelo MCP.

## Hierarquia de execução (OBRIGATÓRIO)

**Sempre verificar se a operação é suportada pelo MCP antes de usar scripts Python.**

### Usar o MCP (`mcp__claude_ai_Meta__ads_*`)

| Operação | Ferramenta MCP |
|---|---|
| Listar contas | `ads_get_ad_accounts` |
| Buscar campanhas / ad sets / ads / conta | `ads_get_ad_entities` |
| Insights e métricas | `ads_get_ad_entities` com `date_preset` |
| Listar criativos | `ads_get_creatives` |
| Preview de criativo | `ads_get_ad_preview` |
| Listar imagens | `ads_get_ad_images` |
| Listar vídeos | `ads_get_ad_videos` |
| Criar campanha | `ads_create_campaign` |
| Criar ad set | `ads_create_ad_set` |
| Criar ad | `ads_create_ad` |
| Criar criativo | `ads_create_creative` |
| Ativar / pausar / editar objetos | `ads_update_entity` / `ads_activate_entity` |
| Audiences customizadas | `ads_create_custom_audience`, `ads_update_custom_audience` |
| Busca no Ad Library | `ads_library_search` |

### Usar scripts Python (fallback — MCP não suporta)

| Operação | Script |
|---|---|
| Swap de url_tags em criativos existentes | `advanced.py swap-url-tags` |
| Duplicar campanha completa (deep copy) | `advanced.py duplicate-campaign --deep` |
| Duplicar ad set ou ad | `advanced.py duplicate-adset / duplicate-ad` |
| Targeting: buscar interesses, comportamentos, geo | `targeting.py` |
| Upload de imagem/vídeo via URL | `create.py image / video` |
| Insights com breakdowns avançados (age/gender/país) | `insights.py` com `--breakdowns` |
| Log de atividades da conta | `read.py activities` |
