...
**MCP TEM PRIORIDADE.** Tentar primeiro via MCP (`mcp__claude_ai_Meta__*`). Usar scripts Python da skill meta-ads-ratos como FALLBACK quando MCP não estiver disponível ou não suportar a operação:
```bash
...
### PASSO 0 — Identificar cliente, plataforma, período e design
...
7. **OBRIGATÓRIO — Perguntar o estilo de design**:

   > "Qual estilo de design para o relatório: **arttico** (padrão), **futurista** ou **minimalista**?"

   Todos os estilos usam as cores base da Arttico (`#00002c` fundo, `#0a0a3d` cards).
   O que muda é tipografia, layout e decoração:

   | Estilo | Fontes | Visual |
   |---|---|---|
   | **arttico** | Montserrat 800 + Inter | Cards com sombra, espaço negativo, sem bordas visíveis |
   | **futurista** | Space Grotesk + Space Mono | Grid de fundo, glow sutil, glassmorphism, badges com borda |
   | **minimalista** | DM Sans 300/600 + DM Mono | Sem cards, só linhas finas, KPIs em strip, valores mono grandes |

   Referências em: `C:\Users\Lucas Felipe\Downloads\Arttico\ccos-ratos\clientes\prime-gourmet\relatorios\`
   - arttico: `relatorio-prime-gourmet-maio2026.html`
   - futurista: `relatorio-prime-gourmet-jan-jun2026-v2.html`
   - minimalista: `relatorio-prime-gourmet-jan-jun2026-v3.html`

8. **Design guide do cliente**: verificar se existe `marca/design-guide.md` no workspace
   (CC-OS RATOS). Se existir, ler e usar as cores do cliente sobrepostas ao estilo escolhido.
...
- **MCP TEM PRIORIDADE** — tentar via MCP primeiro; scripts Python das skills Ratos como fallback
